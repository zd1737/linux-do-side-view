...
const FRAME_LAYOUT_OBSERVER_IDLE_MS = 2000; // iframe 结构稳定后多久停止布局观察
...
let frameLayoutObserver = null; // 监听 iframe 内结构变化，按需补布局同步
let frameLayoutObserverStopTimer = null; // 停止 iframe 布局观察的计时器
...
function initTopLevel() {
  ...
  document.addEventListener("focusin", handleTopLevelFocusIn, true);
  document.documentElement.addEventListener("pointerenter", handleTopLevelPointerEnter, { passive: true });
  document.documentElement.addEventListener("pointerleave", handleTopLevelPointerLeave, { passive: true });
  document.addEventListener("visibilitychange", handleTopLevelVisibilityChange);
  window.addEventListener("blur", handleTopLevelWindowBlur);
...
/**
 * 指针重新进入主页面文档时，按当前进入位置重算一次面板激活态。
 */
function handleTopLevelPointerEnter(event) {
  if (!document.documentElement.classList.contains(OPEN_CLASS)) {
    return;
  }
  ...
  setPanelActiveState(isClientXInsidePanel(event.clientX));
}

/**
 * 指针离开页面文档时立即清掉激活态，避免切走标签页后状态残留。
 */
function handleTopLevelPointerLeave() {
  setPanelActiveState(false);
}

/**
 * 标签页隐藏时清掉右侧激活态，避免打开新标签页后旧状态残留。
 */
function handleTopLevelVisibilityChange() {
  if (document.visibilityState === "hidden") {
    setPanelActiveState(false);
  }
}

function handleTopLevelWindowBlur() {
  setPanelActiveState(false);
}

/**
 * 主页面通过 focus 变化判断当前交互是否落在右侧面板内，兼容键盘操作。
 */
function handleTopLevelFocusIn(event) {
  if (!document.documentElement.classList.contains(OPEN_CLASS)) {
    return;
  }
  ...
  setPanelActiveState(Boolean(event.target.closest(`#${PANEL_ID}`)));
}

/**
 * 接收 iframe 发回的低频交互状态消息，同步右侧面板激活态。
 */
function handleTopLevelMessage(event) {
  ...
  if (data.type === "ds-iframe-activity-change") {
    setPanelActiveState(Boolean(data.active));
    return;
  }

  if (data.type === "ds-iframe-interaction") {
    // 兼容旧版 bridge：只要 iframe 内发生交互，就视为右侧面板激活。
    setPanelActiveState(true);
  }
}

/**
 * 根据视口 X 坐标判断指针当前是否位于右侧面板范围内。
 */
function isClientXInsidePanel(clientX) {
  ...
  return clientX >= panelRect.left && clientX <= panelRect.right;
}
...
function ensurePanel() {
  ...
  if (panel && iframe && resizeHandle && fakeScrollbar && fakeScrollbarThumb) {
    bindPanelActivityListeners(panel);
...
/**
 * 绑定右侧面板进入/离开事件，避免依赖全局 mousemove 持续计算。
 */
function bindPanelActivityListeners(panel) {
  if (panel.dataset.dsActivityBound === "true") {
    return;
  }

  panel.addEventListener("pointerenter", handlePanelPointerEnter);
  panel.addEventListener("pointerleave", handlePanelPointerLeave);
  panel.dataset.dsActivityBound = "true";
}
...
/**
 * 仅在 iframe DOM 结构实际变化时重新调度布局，同步替代固定多次 timeout 轮询。
 */
function startFrameLayoutObservation() {
  if (window.top === window || typeof MutationObserver !== "function" || !scheduleFrameLayout) {
    return;
  }

  if (!frameLayoutObserver) {
    frameLayoutObserver = new MutationObserver((mutations) => {
      ...
      scheduleFrameLayout?.();
      scheduleFrameLayoutObservationStop();
    });

    frameLayoutObserver.observe(document.documentElement, {
      childList: true,
      subtree: true
    });
  }
...
function scheduleFrameLayoutObservationStop() {
  if (frameLayoutObserverStopTimer !== null) {
    window.clearTimeout(frameLayoutObserverStopTimer);
  }

  frameLayoutObserverStopTimer = window.setTimeout(() => {
    stopFrameLayoutObservation();
  }, FRAME_LAYOUT_OBSERVER_IDLE_MS);
}

function stopFrameLayoutObservation() {
  if (frameLayoutObserver) {
    frameLayoutObserver.disconnect();
    frameLayoutObserver = null;
  }

  if (frameLayoutObserverStopTimer !== null) {
    window.clearTimeout(frameLayoutObserverStopTimer);
    frameLayoutObserverStopTimer = null;
  }
}
